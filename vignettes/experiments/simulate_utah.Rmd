...
```{r district-group-risk-summary}
nsims <- 100000
district <- "Granite"
R0 <- 7
sim <- multigroup.vaccine:::simulateOutbreaks(model,
                                              R0=7, 
                                              district_name = district,
                                              initmode = "random",
                                              nsims = 100000)

getGroupStats <- function(sim){
  nsims <- length(sim$init_group)
  
  mean_group_inf <- colSums(sim$final_size) / nsims
  percent_group_init <- tabulate(sim$init_group, ncol(sim$final_size)) / nsims * 100
  percent_group_inf <- colSums(sim$final_size > 0) / nsims * 100
  
  dfgrp <- data.frame(group = sim$group_names, percent_group_init, percent_group_inf, mean_group_inf)
  dfgrp[order(-dfgrp$mean_group_inf), ]
}

dfGranite <- getGroupStats(sim)
knitr::kable(dfGranite, digits = 2, row.names = FALSE)
```
